//! OpenCode Console workspace surface.
//!
//! OpenCode migrated their web workspace UI to a `console.opencode.ai`-style
//! surface (`/console/api/...`) backed by microcent meters. Accounts that
//! migrated no longer have usable data on the old `/workspace/<id>/go` page
//! that [`super::legacy`] scrapes; this module talks to the new surface
//! directly. Ported from Win-CodexBar's Console fallback (upstream v0.64.0).
...
/// Monthly window length used when the payload gives no better estimate.
const DEFAULT_MONTHLY_WINDOW_MINUTES: u32 = 43_200;
...
fn parse_usage(text: &str) -> Result<ConsoleUsage, ProviderError> {
    ...
    if let Some(monthly) = meters.get("month") {
        ...
        snapshot = snapshot
            .with_tertiary(meter_window(
                monthly,
                Some(DEFAULT_MONTHLY_WINDOW_MINUTES),
                resets_at,
            )?)
            // Give the monthly bar its real label instead of the generic
            // "Extra" fallback — same fix as the legacy scraper (#197).
...
fn meter_window(
    ...
    // Missing/unparsable resetsAt stays None — an unknown schedule must never
    // be reported as "resets now" (same principle as the legacy fix, #213).
...
fn parse_balance(text: &str) -> Result<Option<f64>, ProviderError> {
    ...
    // The balance is remaining prepaid credit, not spend — the caller must
    // surface it as an informational line only, never as CostSnapshot (#208).
...
mod tests {
    ...
    fn parses_console_microcent_usage_and_nullable_resets() {
        let ConsoleUsage::Snapshot(snapshot) = parse_usage(&usage_json("null")).unwrap() else {
        ...
        assert_eq!(snapshot.tertiary_label.as_deref(), Some("Monthly"));
    }
    ...
    fn distinguishes_no_subscription_from_malformed_usage() {
        ...
        assert!(matches!(
            parse_usage(r#"{"access":null}"#),
            Ok(ConsoleUsage::NoSubscription)
        ));
        assert!(matches!(
            parse_usage(r#"{"access":{}}"#),
